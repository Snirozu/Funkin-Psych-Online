...
import hxluau.Types.Lua_State;
#end

class LuaModuleSwap {
    static final DAY_IN_MS = 1000 * 60 * 60 * 24;

    public static function doLua(lua:State, originPath:String, ?stopFunc:Void->Void) {
        ...
        set(lua, 'require', function(?module:String):Dynamic {
            ...
            if (FileSystem.exists(filePath(module))) {
                ...
                Lua.pushvalue(lua, Lua.GLOBALSINDEX); // Get global table
                Lua.pushnil(lua); // put a nil key on stack
                while (Lua.next(lua, -2) != 0) { // key(-1) is replaced by the next key(-1) in table(-2)
                    var rawStr = Lua.tostring(lua, -2);
                    if (rawStr != null) fields.push(rawStr.toString()); // Get key(-2) name
    ...
    static function set(lua:State, name:String, value:Dynamic) {
        if (Reflect.isFunction(value)) {
            Convert.addCallback(lua, name, value);
            return;
        }
...
class LumodModuleAddon extends lumod.addons.LumodAddon {
    override function init() {
		var luaObj:Dynamic = instance.lua;
		var statePointer:cpp.RawPointer<hxluau.Types.Lua_State> = 
			cast cpp.Pointer.fromRaw(cpp.RawPointer.addressOf(luaObj)).raw;

		LuaModuleSwap.doLua(statePointer, instance.__scriptPath);
    }
}
#end